...
| SF-6 | Low      | No Content-Security-Policy on the SPA host (Helmet covers the API; the static frontend needs its own CSP headers).       | ✅ **Resolved (Phase 6)** — API CSP `default-src 'none'`; SPA CSP + security headers served by the nginx edge (§8.3). Prod swaps `connect-src` to the real API origin. |
